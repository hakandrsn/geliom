...
function MemberCard({ member, isMe, onPress }: MemberCardProps) {
  ...
  }, [member.updatedAt]); // Update zamanı değiştiğinde tetikle

  // Mood değiştiğinde scale efekti
  useEffect(() => {
    if (member.moodEmoji) {
      moodScale.value = withSequence(withSpring(1.5), withSpring(1));
    }
  }, [member.updatedAt]);
  ...
  const displayName =
    member.nickname || member.displayName || member.customId || "";
  const realName = member.nickname
    ? member.displayName || member.customId
    : undefined;

  const statusColor = colors.secondaryText; // Simplifying color logic for now
  ...
  const cardContent = (
    ...
    >
      ...
      <View style={styles.contentWrapper}>
        <View
          style={[styles.avatarContainer, { backgroundColor: colors.tertiary }]}
        >
          <Image
          ...
          {member.moodEmoji && (
            <Animated.View style={[styles.moodBadge, animatedMoodStyle]}>
              <Typography variant="h6">{member.moodEmoji}</Typography>
            </Animated.View>
          )}
        </View>
