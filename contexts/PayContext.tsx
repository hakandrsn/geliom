...
const activateAdapty = async (): Promise<void> => {
  ...
  activationPromise = (async () => {
    ...
    try {
      ...
      const activationPromise = adapty.activate(key, {
        lockMethodsUntilReady: false,
      });
  ...
  const showPaywall = async (options: ShowPaywallOptions) => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          "Paywall products fetched:",
          JSON.stringify(products, null, 2)
        );
      } catch (productError) {
        console.error(
          "Error fetching paywall products explicitly:",
          productError
        );
      }
      ...
      view.setEventHandlers({
        onPurchaseCompleted: (purchase: any) => {
          if (purchase?.profile?.accessLevels?.["premium"]?.isActive) {
            ...
            onFailure?.(
              new Error("Purchase succeeded but no access level was granted.")
            );
          }
          view.dismiss();
        },
